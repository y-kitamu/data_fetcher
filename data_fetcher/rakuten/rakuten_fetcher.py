...
def get_available_tickers(data_dir: Path) -> list[str]:
    """ """
    return sorted(set([csv_path.stem.split("_")[0] for csv_path in data_dir.rglob("*.csv")]))
...
class RakutenFetcher(BaseFetcher):
    ...
    def get_latest_date(self, symbol: str) -> datetime.datetime:
        ...
        return datetime.datetime.strptime(csv_list[-1].parent.name, "%Y%m%d") + datetime.timedelta(
            days=1
        )

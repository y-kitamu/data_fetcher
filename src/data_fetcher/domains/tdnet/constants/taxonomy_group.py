from pydantic import BaseModel


class TmpTaxonomy(BaseModel):
    element_id: str
    namespace: str
    japanese_label: str
    english_label: str
    period_type: str

    @property
    def full_element_id(self) -> str:
        return f"{self.namespace}.{self.element_id}"


taxonomy_groups = {
    "cash_and_equivalents": [
        TmpTaxonomy(
            japanese_label="現金及び現金同等物期末残高",
            english_label="Cash and equivalents, end of period",
            namespace="tse-ed-t",
            element_id="CashAndEquivalentsEndOfPeriod",
            period_type="instant",
        ),
        TmpTaxonomy(
            japanese_label="現金及び預金",
            english_label="Cash and deposits",
            namespace="jppfs_cor",
            element_id="CashAndDeposits",
            period_type="instant",
        ),
        TmpTaxonomy(
            japanese_label="現金及び現金同等物（IFRS）",
            english_label="Cash and cash equivalents (IFRS)",
            namespace="jpigp_cor",
            element_id="CashAndCashEquivalentsIFRS",
            period_type="instant",
        ),
        TmpTaxonomy(
            japanese_label="現金及び現金同等物期末残高、米国基準",
            english_label="Cash and equivalents, end of period-US",
            namespace="tse-ed-t",
            element_id="CashAndEquivalentsEndOfPeriodUS",
            period_type="instant",
        ),
    ],
    "receivables": [
        TmpTaxonomy(
            japanese_label="受取手形",
            english_label="Notes receivable-trade",
            namespace="jppfs_cor",
            element_id="NotesReceivableTrade",
            period_type="instant",
        ),
        TmpTaxonomy(
            japanese_label="売掛金及びその他の債権、資産（IFRS）",
            english_label="Trade and other receivables - 3 - Assets (IFRS)",
            namespace="jpigp_cor",
            element_id="TradeAndOtherReceivables3AssetsIFRS",
            period_type="instant",
        ),
    ],
    "inventory": [
        TmpTaxonomy(
            japanese_label="棚卸資産、流動資産（IFRS）",
            english_label="Inventories - CA (IFRS)",
            namespace="jpigp_cor",
            element_id="InventoriesCAIFRS",
            period_type="instant",
        ),
        TmpTaxonomy(
            japanese_label="棚卸資産",
            english_label="Inventories",
            namespace="jppfs_cor",
            element_id="Inventories",
            period_type="instant",
        ),
    ],
    "current_assets": [
        TmpTaxonomy(
            japanese_label="流動資産（IFRS）",
            english_label="Current assets (IFRS)",
            namespace="jpigp_cor",
            element_id="CurrentAssetsIFRS",
            period_type="instant",
        ),
        TmpTaxonomy(
            japanese_label="流動資産",
            english_label="Current assets",
            namespace="jppfs_cor",
            element_id="CurrentAssets",
            period_type="instant",
        ),
    ],
    "ppe": [
        TmpTaxonomy(
            japanese_label="有形固定資産",
            english_label="Property, plant and equipment",
            namespace="jppfs_cor",
            element_id="PropertyPlantAndEquipment",
            period_type="instant",
        ),
        TmpTaxonomy(
            japanese_label="有形固定資産（IFRS）",
            english_label="Property, plant and equipment (IFRS)",
            namespace="jpigp_cor",
            element_id="PropertyPlantAndEquipmentIFRS",
            period_type="instant",
        ),
    ],
    "intangible_assets": [
        TmpTaxonomy(
            japanese_label="無形固定資産",
            english_label="Intangible assets",
            namespace="jppfs_cor",
            element_id="IntangibleAssets",
            period_type="instant",
        ),
        TmpTaxonomy(
            japanese_label="無形資産（IFRS）",
            english_label="Intangible assets (IFRS)",
            namespace="jpigp_cor",
            element_id="IntangibleAssetsIFRS",
            period_type="instant",
        ),
    ],
    "investment_securities": [
        TmpTaxonomy(
            japanese_label="投資有価証券、非流動資産（IFRS）",
            english_label="Investment securities - NCA (IFRS)",
            namespace="jpigp_cor",
            element_id="InvestmentSecuritiesNCAIFRS",
            period_type="instant",
        ),
        TmpTaxonomy(
            japanese_label="投資有価証券",
            english_label="Investment securities",
            namespace="jppfs_cor",
            element_id="InvestmentSecurities",
            period_type="instant",
        ),
    ],
    "non_current_assets": [
        TmpTaxonomy(
            japanese_label="固定資産",
            english_label="Non-current assets",
            namespace="jppfs_cor",
            element_id="NoncurrentAssets",
            period_type="instant",
        ),
        TmpTaxonomy(
            japanese_label="非流動資産（IFRS）",
            english_label="Non-current assets (IFRS)",
            namespace="jpigp_cor",
            element_id="NonCurrentAssetsIFRS",
            period_type="instant",
        ),
    ],
    "total_assets": [
        TmpTaxonomy(
            japanese_label="資産（IFRS）",
            english_label="Assets (IFRS)",
            namespace="jpigp_cor",
            element_id="AssetsIFRS",
            period_type="instant",
        ),
        TmpTaxonomy(
            japanese_label="資産",
            english_label="Assets",
            namespace="jppfs_cor",
            element_id="Assets",
            period_type="instant",
        ),
        TmpTaxonomy(
            japanese_label="総資産",
            english_label="Total assets",
            namespace="tse-ed-t",
            element_id="TotalAssets",
            period_type="instant",
        ),
    ],
    "payables": [
        TmpTaxonomy(
            japanese_label="買掛金及びその他の債務、流動負債（IFRS）",
            english_label="Trade and other payables - 3 - CL (IFRS)",
            namespace="jpigp_cor",
            element_id="TradeAndOtherPayables3CLIFRS",
            period_type="instant",
        ),
        TmpTaxonomy(
            japanese_label="支払手形",
            english_label="Notes payable-trade",
            namespace="jppfs_cor",
            element_id="NotesPayableTrade",
            period_type="instant",
        ),
    ],
    "short_term_borrowings": [
        TmpTaxonomy(
            japanese_label="短期借入金",
            english_label="Short-term borrowings",
            namespace="jppfs_cor",
            element_id="ShortTermLoansPayable",
            period_type="instant",
        ),
        TmpTaxonomy(
            japanese_label="１年内返済予定の長期借入金（IFRS）",
            english_label="Current portion of long-term borrowings - CL (IFRS)",
            namespace="jpigp_cor",
            element_id="CurrentPortionOfLongTermBorrowingsCLIFRS",
            period_type="instant",
        ),
    ],
    "current_liabilities": [
        TmpTaxonomy(
            japanese_label="流動負債（IFRS）",
            english_label="Current liabilities (IFRS)",
            namespace="jpigp_cor",
            element_id="TotalCurrentLiabilitiesIFRS",
            period_type="instant",
        ),
        TmpTaxonomy(
            japanese_label="流動負債",
            english_label="Current liabilities",
            namespace="jppfs_cor",
            element_id="CurrentLiabilities",
            period_type="instant",
        ),
    ],
    "long_term_borrowings": [
        TmpTaxonomy(
            japanese_label="長期借入金",
            english_label="Long-term borrowings",
            namespace="jppfs_cor",
            element_id="LongTermLoansPayable",
            period_type="instant",
        ),
        TmpTaxonomy(
            japanese_label="長期債務、非流動負債（IFRS）",
            english_label="Long-term debt - NCL (IFRS)",
            namespace="jpigp_cor",
            element_id="LongTermDebtNCLIFRS",
            period_type="instant",
        ),
    ],
    "non_current_liabilities": [
        TmpTaxonomy(
            japanese_label="固定負債",
            english_label="Non-current liabilities",
            namespace="jppfs_cor",
            element_id="NoncurrentLiabilities",
            period_type="instant",
        ),
        TmpTaxonomy(
            japanese_label="非流動負債（IFRS）",
            english_label="Non-current liabilities (IFRS)",
            namespace="jpigp_cor",
            element_id="NonCurrentLabilitiesIFRS",  # typo
            period_type="instant",
        ),
    ],
    "liabilities": [
        TmpTaxonomy(
            japanese_label="負債（IFRS）",
            english_label="Liabilities (IFRS)",
            namespace="jpigp_cor",
            element_id="LiabilitiesIFRS",
            period_type="instant",
        ),
        TmpTaxonomy(
            japanese_label="負債",
            english_label="Liabilities",
            namespace="jppfs_cor",
            element_id="Liabilities",
            period_type="instant",
        ),
    ],
    "shareholders_equity": [
        TmpTaxonomy(
            japanese_label="株主資本",
            english_label="Shareholders' equity",
            namespace="jppfs_cor",
            element_id="ShareholdersEquity",
            period_type="instant",
        ),
        TmpTaxonomy(
            japanese_label="親会社の所有者に帰属する持分（IFRS）",
            english_label="Equity attributable to owners of parent (IFRS)",
            namespace="jpigp_cor",
            element_id="EquityAttributableToOwnersOfParentIFRS",
            period_type="instant",
        ),
    ],
    "net_assets": [
        TmpTaxonomy(
            japanese_label="純資産",
            english_label="Net assets",
            namespace="jppfs_cor",
            element_id="NetAssets",
            period_type="instant",
        ),
        TmpTaxonomy(
            japanese_label="資本（IFRS）",
            english_label="Equity (IFRS)",
            namespace="jpigp_cor",
            element_id="EquityIFRS",
            period_type="instant",
        ),
        TmpTaxonomy(
            japanese_label="純資産",
            english_label="Net assets",
            namespace="tse-ed-t",
            element_id="NetAssets",
            period_type="instant",
        ),
    ],
    "liabilities_and_net_assets": [
        TmpTaxonomy(
            japanese_label="負債純資産",
            english_label="Liabilities and net assets",
            namespace="jppfs_cor",
            element_id="LiabilitiesAndNetAssets",
            period_type="instant",
        ),
        TmpTaxonomy(
            japanese_label="負債及び資本（IFRS）",
            english_label="Liabilities and equity (IFRS)",
            namespace="jpigp_cor",
            element_id="LiabilitiesAndEquityIFRS",
            period_type="instant",
        ),
    ],
    "net_sales": [
        TmpTaxonomy(
            japanese_label="売上高",
            english_label="Net sales",
            namespace="tse-ed-t",
            element_id="NetSales",
            period_type="duration",
        ),
        TmpTaxonomy(
            japanese_label="売上高（IFRS）",
            english_label="Net sales (IFRS)",
            namespace="tse-ed-t",
            element_id="NetSalesIFRS",
            period_type="duration",
        ),
        TmpTaxonomy(
            japanese_label="売上高、米国基準",
            english_label="Net sales-US",
            namespace="tse-ed-t",
            element_id="NetSalesUS",
            period_type="duration",
        ),
        TmpTaxonomy(
            japanese_label="売上高（IFRS、旧タクソノミ）",
            english_label="Sales (IFRS, legacy taxonomy)",
            namespace="tse-ed-t",
            element_id="SalesIFRS",
            period_type="duration",
        ),
        TmpTaxonomy(
            japanese_label="営業収益",
            english_label="Operating revenues",
            namespace="tse-ed-t",
            element_id="OperatingRevenues",
            period_type="duration",
        ),
        TmpTaxonomy(
            japanese_label="営業収益（IFRS）",
            english_label="Operating revenues (IFRS)",
            namespace="tse-ed-t",
            element_id="OperatingRevenuesIFRS",
            period_type="duration",
        ),
        TmpTaxonomy(
            japanese_label="売上高（IFRS）",
            english_label="Net sales (IFRS)",
            namespace="jpigp_cor",
            element_id="NetSalesIFRS",
            period_type="duration",
        ),
        TmpTaxonomy(
            japanese_label="売上高",
            english_label="Net sales",
            namespace="jppfs_cor",
            element_id="NetSales",
            period_type="duration",
        ),
    ],
    "cost_of_sales": [
        TmpTaxonomy(
            japanese_label="売上原価",
            english_label="Cost of sales",
            namespace="jppfs_cor",
            element_id="CostOfSales",
            period_type="duration",
        ),
        TmpTaxonomy(
            japanese_label="売上原価（IFRS）",
            english_label="Cost of sales (IFRS)",
            namespace="jpigp_cor",
            element_id="CostOfSalesIFRS",
            period_type="duration",
        ),
    ],
    "sga_expenses": [
        TmpTaxonomy(
            japanese_label="販売費及び一般管理費",
            english_label="Selling, general and administrative expenses",
            namespace="jppfs_cor",
            element_id="SellingGeneralAndAdministrativeExpenses",
            period_type="duration",
        ),
        TmpTaxonomy(
            japanese_label="販売費及び一般管理費（IFRS）",
            english_label="Selling, general and administrative expenses (IFRS)",
            namespace="jpigp_cor",
            element_id="SellingGeneralAndAdministrativeExpensesIFRS",
            period_type="duration",
        ),
    ],
    "operating_profit": [
        TmpTaxonomy(
            japanese_label="営業利益（△損失）（IFRS）",
            english_label="Operating profit (loss) (IFRS)",
            namespace="jpigp_cor",
            element_id="OperatingProfitLossIFRS",
            period_type="duration",
        ),
        TmpTaxonomy(
            japanese_label="営業利益又は営業損失（△）",
            english_label="Operating profit (loss)",
            namespace="jppfs_cor",
            element_id="OperatingIncome",
            period_type="duration",
        ),
        TmpTaxonomy(
            japanese_label="営業利益",
            english_label="Operating profit",
            namespace="tse-ed-t",
            element_id="OperatingIncome",
            period_type="duration",
        ),
        TmpTaxonomy(
            japanese_label="営業利益（IFRS）",
            english_label="Operating profit (IFRS)",
            namespace="tse-ed-t",
            element_id="OperatingIncomeIFRS",
            period_type="duration",
        ),
        TmpTaxonomy(
            japanese_label="営業利益、米国基準",
            english_label="Operating profit-US",
            namespace="tse-ed-t",
            element_id="OperatingIncomeUS",
            period_type="duration",
        ),
    ],
    "ordinary_profit": [
        TmpTaxonomy(
            japanese_label="経常利益又は経常損失（△）",
            english_label="Ordinary profit (loss)",
            namespace="jppfs_cor",
            element_id="OrdinaryIncome",
            period_type="duration",
        ),
        TmpTaxonomy(
            japanese_label="経常利益",
            english_label="Ordinary profit",
            namespace="tse-ed-t",
            element_id="OrdinaryIncome",
            period_type="duration",
        ),
    ],
    "net_income": [
        TmpTaxonomy(
            japanese_label="当期純利益",
            english_label="Net income",
            namespace="tse-ed-t",
            element_id="NetIncome",
            period_type="duration",
        ),
        TmpTaxonomy(
            japanese_label="親会社株主に帰属する当期純利益",
            english_label="Profit attributable to owners of parent",
            namespace="tse-ed-t",
            element_id="ProfitAttributableToOwnersOfParent",
            period_type="duration",
        ),
        TmpTaxonomy(
            japanese_label="親会社の所有者に帰属する当期利益（IFRS）",
            english_label="Profit attributable to owners of parent (IFRS)",
            namespace="tse-ed-t",
            element_id="ProfitAttributableToOwnersOfParentIFRS",
            period_type="duration",
        ),
        TmpTaxonomy(
            japanese_label="当期純利益、米国基準",
            english_label="Net income-US",
            namespace="tse-ed-t",
            element_id="NetIncomeUS",
            period_type="duration",
        ),
        TmpTaxonomy(
            japanese_label="当期利益（△損失）（IFRS）",
            english_label="Profit (loss) (IFRS)",
            namespace="jpigp_cor",
            element_id="ProfitLossIFRS",
            period_type="duration",
        ),
        TmpTaxonomy(
            japanese_label="当期純利益又は当期純損失（△）（平成26年3月28日財規等改正後）",
            english_label="Profit (loss) (after amendment dated 2014-03-28)",
            namespace="jppfs_cor",
            element_id="ProfitLoss",
            period_type="duration",
        ),
    ],
    "cash_flow_from_operating_activities": [
        TmpTaxonomy(
            japanese_label="営業活動によるキャッシュ・フロー、米国基準",
            english_label="Cash flows from operating activities-US",
            namespace="tse-ed-t",
            element_id="CashFlowsFromOperatingActivitiesUS",
            period_type="duration",
        ),
        TmpTaxonomy(
            japanese_label="営業活動によるキャッシュ・フロー、IFRS",
            english_label="Cash flows from operating activities-IFRS",
            namespace="tse-ed-t",
            element_id="CashFlowsFromOperatingActivitiesIFRS",
            period_type="duration",
        ),
        TmpTaxonomy(
            japanese_label="営業活動によるキャッシュ・フロー",
            english_label="Cash flows from operating activities",
            namespace="tse-ed-t",
            element_id="CashFlowsFromOperatingActivities",
            period_type="duration",
        ),
        TmpTaxonomy(
            japanese_label="営業活動によるキャッシュ・フロー",
            english_label="Net cash provided by (used in) operating activities",
            namespace="jppfs_cor",
            element_id="NetCashProvidedByUsedInOperatingActivities",
            period_type="duration",
        ),
        TmpTaxonomy(
            japanese_label="営業活動によるキャッシュ・フロー（IFRS）",
            english_label="Net cash provided by (used in) operating activities (IFRS)",
            namespace="jpigp_cor",
            element_id="NetCashProvidedByUsedInOperatingActivitiesIFRS",
            period_type="duration",
        ),
    ],
    "depreciation": [
        TmpTaxonomy(
            japanese_label="減価償却費、営業活動によるキャッシュ・フロー",
            english_label="Depreciation-OpeCF",
            namespace="jppfs_cor",
            element_id="DepreciationAndAmortizationOpeCF",
            period_type="duration",
        )
    ],
    "cash_flow_from_investing_activities": [
        TmpTaxonomy(
            japanese_label="投資活動によるキャッシュ・フロー",
            english_label="Cash flows from investing activities",
            namespace="tse-ed-t",
            element_id="CashFlowsFromInvestingActivities",
            period_type="duration",
        ),
        TmpTaxonomy(
            japanese_label="投資活動によるキャッシュ・フロー、IFRS",
            english_label="Cash flows from investing activities-IFRS",
            namespace="tse-ed-t",
            element_id="CashFlowsFromInvestingActivitiesIFRS",
            period_type="duration",
        ),
        TmpTaxonomy(
            japanese_label="投資活動によるキャッシュ・フロー、米国基準",
            english_label="Cash flows from investing activities-US",
            namespace="tse-ed-t",
            element_id="CashFlowsFromInvestingActivitiesUS",
            period_type="duration",
        ),
        TmpTaxonomy(
            japanese_label="投資活動によるキャッシュ・フロー",
            english_label="Net cash provided by (used in) investing activities",
            namespace="jppfs_cor",
            element_id="NetCashProvidedByUsedInInvestmentActivities",
            period_type="duration",
        ),
        TmpTaxonomy(
            japanese_label="投資活動によるキャッシュ・フロー（IFRS）",
            english_label="Net cash provided by (used in) investing activities (IFRS)",
            namespace="jpigp_cor",
            element_id="NetCashProvidedByUsedInInvestingActivitiesIFRS",
            period_type="duration",
        ),
    ],
    "capital_investment": [
        TmpTaxonomy(
            japanese_label="有形固定資産の取得による支出、投資活動によるキャッシュ・フロー",
            english_label="Purchase of property, plant and equipment-InvCF",
            namespace="jppfs_cor",
            element_id="PurchaseOfPropertyPlantAndEquipmentInvCF",
            period_type="duration",
        ),
        TmpTaxonomy(
            japanese_label="有形固定資産の取得による支出",
            english_label="Purchase of property, plant and equipment - InvCF (IFRS)",
            namespace="jpigp_cor",
            element_id="PurchaseOfPropertyPlantAndEquipmentInvCFIFRS",
            period_type="duration",
        ),
    ],
    "cash_flow_from_financial_activities": [
        TmpTaxonomy(
            japanese_label="財務活動によるキャッシュ・フロー、米国基準",
            english_label="Cash flows from financing activities-US",
            namespace="tse-ed-t",
            element_id="CashFlowsFromFinancingActivitiesUS",
            period_type="duration",
        ),
        TmpTaxonomy(
            japanese_label="財務活動によるキャッシュ・フロー、IFRS",
            english_label="Cash flows from financing activities-IFRS",
            namespace="tse-ed-t",
            element_id="CashFlowsFromFinancingActivitiesIFRS",
            period_type="duration",
        ),
        TmpTaxonomy(
            japanese_label="財務活動によるキャッシュ・フロー",
            english_label="Cash flows from financing activities",
            namespace="tse-ed-t",
            element_id="CashFlowsFromFinancingActivities",
            period_type="duration",
        ),
        TmpTaxonomy(
            japanese_label="財務活動によるキャッシュ・フロー（IFRS）",
            english_label="Net cash provided by (used in) financing activities (IFRS)",
            namespace="jpigp_cor",
            element_id="NetCashProvidedByUsedInFinancingActivitiesIFRS",
            period_type="duration",
        ),
        TmpTaxonomy(
            japanese_label="財務活動によるキャッシュ・フロー",
            english_label="Net cash provided by (used in) financing activities",
            namespace="jppfs_cor",
            element_id="NetCashProvidedByUsedInFinancingActivities",
            period_type="duration",
        ),
    ],
    "number_of_shares": [
        TmpTaxonomy(
            japanese_label="期末発行済株式数（自己株式を含む）",
            english_label="Number of issued and outstanding shares at the end of fiscal year (including treasury stock)",
            namespace="tse-ed-t",
            element_id="NumberOfIssuedAndOutstandingSharesAtTheEndOfFiscalYearIncludingTreasuryStock",
            period_type="instant",
        ),
    ],
    "eps": [
        TmpTaxonomy(
            japanese_label="1株当たり当期純利益",
            english_label="Net income per share",
            namespace="tse-ed-t",
            element_id="NetIncomePerShare",
            period_type="duration",
        ),
        TmpTaxonomy(
            japanese_label="1株当たり当期純利益、米国基準",
            english_label="Net income per share-US",
            namespace="tse-ed-t",
            element_id="NetIncomePerShareUS",
            period_type="duration",
        ),
        TmpTaxonomy(
            japanese_label="基本的1株当たり当期利益（IFRS）",
            english_label="Basic earnings per share (IFRS)",
            namespace="tse-ed-t",
            element_id="BasicEarningsPerShareIFRS",
            period_type="duration",
        ),
        TmpTaxonomy(
            japanese_label="基本的1株当たり当期利益（IFRS）",
            english_label="Basic earnings (loss) per share (IFRS)",
            namespace="jpigp_cor",
            element_id="BasicEarningsLossPerShareIFRS",
            period_type="duration",
        ),
        TmpTaxonomy(
            japanese_label="基本的及び希薄化後１株当たり当期利益（△損失）（IFRS）",
            english_label="Basic and diluted earnings (loss) per share (IFRS)",
            namespace="jpigp_cor",
            element_id="BasicAndDilutedEarningsLossPerShareIFRS",
            period_type="duration",
        ),
        TmpTaxonomy(
            japanese_label="1株当たり当期純利益、米国基準",
            english_label="Basic net income per share-US",
            namespace="tse-ed-t",
            element_id="BasicNetIncomePerShareUS",
            period_type="duration",
        ),
    ],
    "bps": [
        TmpTaxonomy(
            japanese_label="1株当たり純資産額",
            english_label="Net assets per share",
            namespace="tse-ed-t",
            element_id="NetAssetsPerShare",
            period_type="instant",
        ),
        TmpTaxonomy(
            japanese_label="1株当たり親会社所有者帰属持分（IFRS）",
            english_label="Equity attributable to owners of parent per share (IFRS)",
            namespace="tse-ed-t",
            element_id="EquityAttributableToOwnersOfParentPerShareIFRS",
            period_type="instant",
        ),
        TmpTaxonomy(
            japanese_label="1株当たり純資産額、米国基準",
            english_label="Shareholders' equity per share-US",
            namespace="tse-ed-t",
            element_id="ShareholdersEquityPerShareUS",
            period_type="instant",
        ),
    ],
    "roe": [
        TmpTaxonomy(
            japanese_label="自己資本利益率",
            english_label="Net income to shareholders' equity ratio",
            namespace="tse-ed-t",
            element_id="NetIncomeToShareholdersEquityRatio",
            period_type="duration",
        ),
        TmpTaxonomy(
            japanese_label="自己資本利益率、米国基準",
            english_label="Net income to shareholders' equity ratio-US",
            namespace="tse-ed-t",
            element_id="NetIncomeToShareholdersEquityRatioUS",
            period_type="duration",
        ),
        TmpTaxonomy(
            japanese_label="少数株主損益調整前当期純利益に対する連結自己資本比率、米国基準",
            english_label="Income before minority interest to consolidated equity ratio-US",
            namespace="tse-ed-t",
            element_id="IncomeBeforeMinorityInterestToConsolidatedEquityRatioUS",
            period_type="duration",
        ),
    ],
    "dividend_per_share": [
        TmpTaxonomy(
            japanese_label="1株当たり配当金",
            english_label="Dividend per share",
            namespace="tse-ed-t",
            element_id="DividendPerShare",
            period_type="duration",
        ),
    ],
    "revenue_yoy": [
        TmpTaxonomy(
            japanese_label="売上高増減率",
            english_label="Change in net sales",
            namespace="tse-ed-t",
            element_id="ChangeInNetSales",
            period_type="duration",
        ),
        TmpTaxonomy(
            japanese_label="売上高増減率（IFRS）",
            english_label="Change in net sales (IFRS)",
            namespace="tse-ed-t",
            element_id="ChangeInNetSalesIFRS",
            period_type="duration",
        ),
        TmpTaxonomy(
            japanese_label="売上高増減率、米国基準",
            english_label="Change in net sales-US",
            namespace="tse-ed-t",
            element_id="ChangeInNetSalesUS",
            period_type="duration",
        ),
        TmpTaxonomy(
            japanese_label="売上高増減率（IFRS、旧タクソノミ）",
            english_label="Change in sales (IFRS, legacy taxonomy)",
            namespace="tse-ed-t",
            element_id="ChangeInSalesIFRS",
            period_type="duration",
        ),
        TmpTaxonomy(
            japanese_label="営業収益増減率",
            english_label="Change in operating revenues",
            namespace="tse-ed-t",
            element_id="ChangeInOperatingRevenues",
            period_type="duration",
        ),
        TmpTaxonomy(
            japanese_label="営業収益増減率（IFRS）",
            english_label="Change in operating revenues (IFRS)",
            namespace="tse-ed-t",
            element_id="ChangeInOperatingRevenuesIFRS",
            period_type="duration",
        ),
    ],
    "operating_income_yoy": [
        TmpTaxonomy(
            japanese_label="営業利益増減率",
            english_label="Change in operating income",
            namespace="tse-ed-t",
            element_id="ChangeInOperatingIncome",
            period_type="duration",
        ),
        TmpTaxonomy(
            japanese_label="営業利益増減率（IFRS）",
            english_label="Change in operating income (IFRS)",
            namespace="tse-ed-t",
            element_id="ChangeInOperatingIncomeIFRS",
            period_type="duration",
        ),
        TmpTaxonomy(
            japanese_label="営業利益増減率、米国基準",
            english_label="Change in operating income-US",
            namespace="tse-ed-t",
            element_id="ChangeInOperatingIncomeUS",
            period_type="duration",
        ),
    ],
    "ordinary_profit_yoy": [
        TmpTaxonomy(
            japanese_label="経常利益増減率",
            english_label="Change in ordinary income",
            namespace="tse-ed-t",
            element_id="ChangeInOrdinaryIncome",
            period_type="duration",
        ),
    ],
    "net_income_yoy": [
        TmpTaxonomy(
            japanese_label="当期純利益増減率",
            english_label="Change in net income",
            namespace="tse-ed-t",
            element_id="ChangeInNetIncome",
            period_type="duration",
        ),
        TmpTaxonomy(
            japanese_label="親会社株主に帰属する当期純利益増減率",
            english_label="Change in profit attributable to owners of parent",
            namespace="tse-ed-t",
            element_id="ChangeInProfitAttributableToOwnersOfParent",
            period_type="duration",
        ),
        TmpTaxonomy(
            japanese_label="親会社の所有者に帰属する当期利益増減率（IFRS）",
            english_label="Change in profit attributable to owners of parent (IFRS)",
            namespace="tse-ed-t",
            element_id="ChangeInProfitAttributableToOwnersOfParentIFRS",
            period_type="duration",
        ),
        TmpTaxonomy(
            japanese_label="当期純利益増減率、米国基準",
            english_label="Change in net income-US",
            namespace="tse-ed-t",
            element_id="ChangeInNetIncomeUS",
            period_type="duration",
        ),
    ],
}

# CSV の `element_id` 列（"namespace:element_id" 形式）をキーとした正規化概念名へのマッピング。
ELEMENT_TO_CONCEPT: dict[str, str] = {
    f"{element.namespace}:{element.element_id}": concept_name
    for concept_name, elements in taxonomy_groups.items()
    for element in elements
}

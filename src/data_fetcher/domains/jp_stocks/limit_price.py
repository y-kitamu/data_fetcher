"""Limit price calculations for Japanese stock market.

This module provides functions to calculate and check limit prices
(制限値幅) for Japanese stocks based on previous closing prices.
"""


def is_limit_high(prev_close: float, target: float) -> bool:
    """Check if the target price is at the upper limit.

    Args:
        prev_close: Previous closing price.
        target: Target price to check.

    Returns:
        True if the target price is at the upper limit, False otherwise.
    """
...
def is_limit_low(prev_close: float, target: float) -> bool:
    """Check if the target price is at the lower limit.
    ...
    Returns:
        True if the target price is at the lower limit, False otherwise.
    """
    if is_limit(prev_close, target):
        return prev_close > target
    return False
...
def is_limit(prev_close: float, target: float) -> bool:
    """Check if the target price is at the limit (upper or lower).
    ...
    Returns:
        True if the target price is at the limit, False otherwise.
    """
...
def get_limit_range(prev_close: float) -> float:
    """Get the limit range based on the previous closing price.

    This function returns the maximum allowed price change (制限値幅)
    based on Japanese stock market rules.

    Args:
        prev_close: Previous closing price.

    Returns:
        The limit range for the given price.
    """
    if prev_close < 100:
        return 30
    elif prev_close < 200:

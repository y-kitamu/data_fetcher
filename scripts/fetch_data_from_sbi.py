...
def fetch_sbi_auth_email(service, after_timestamp):
    ...
    """
    query = (
        "from:info@sbisec.co.jp subject:認証コードのお知らせ（ログイン時の追加認証）"
    )
    ...
    for attempt in range(30):  # 最大60秒 (2秒 × 30回)
        ...
        code_match = re.search(r" (\d{6})\r\n", body)
        if code_match:
            auth_code = code_match.group(1)
            logger.info(f"認証メールの認証コードを取得: {auth_code}")
            return auth_code

        logger.warning("メール本文から認証コードを抽出できませんでした")

    raise TimeoutError("SBI認証メールが60秒以内に届きませんでした")


def login_to_sbi(driver):
    """SBIデバイス認証フローを自動処理する。
    ...
    # 2. Gmail APIでメール受信待機 → 認証コード取得
    logger.info("デバイス認証: 認証メール受信を待機中")
    auth_code = fetch_sbi_auth_email(gmail_service, timestamp_before)

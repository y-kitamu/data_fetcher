...
def fetch_history(
    start_date: datetime.date | None, end_date: datetime.date, sleep_seconds: float
):
    ...
    if start_date is None:
        ...
        start_date, csrf_token = (
            data_fetcher.domains.taisyaku.data.find_earliest_available_date(
    ...
    for target_date in tqdm.tqdm(dates):
        ...
        csv_text, csrf_token = (
            data_fetcher.domains.taisyaku.data.fetch_history_for_date(
...
def main():
    ...
    if args.overlap_days > 0:
        start_date = args.end_date - datetime.timedelta(days=args.overlap_days)

    fetch_history(start_date, args.end_date, args.sleep_seconds)
